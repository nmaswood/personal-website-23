...
class RetroEffectImpl extends Effect {
  ...
  private colorNumUniform: THREE.Uniform;
  private pixelSizeUniform: THREE.Uniform;

  constructor() {
    const colorNumUniform = new THREE.Uniform(4.0);
    const pixelSizeUniform = new THREE.Uniform(2.0);
  ...
  get colorNum(): number {
    return this.colorNumUniform.value as number;
  }
...
>((props, ref) => {
  return <WrappedRetro ref={ref} {...props} />;
});
...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    const res: THREE.Vector2 = waveUniformsRef.current.resolution.value;
    if (res.x !== w || res.y !== h) {
      res.set(w, h);
    }
  }, [size, gl]);
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevColor.current.every((v, i) => v === waveColor[i])) {
      const color: THREE.Color = u.waveColor.value;
      color.set(...waveColor);
    ...
    if (enableMouseInteraction) {
      const pos: THREE.Vector2 = u.mousePos.value;
      pos.copy(mouseRef.current);
    }
  });
